...
<div class="woocommerce-order">
    ...
    ?>
        ...
        <?php else : ?>
            ...
            if ($tickets) {
                echo '<div class="event-tickets" style="margin-top:40px;">';
                echo '<h2>Your Ticket</h2>';

                foreach ($tickets as $ticket) {
                    $verify_url = site_url('/verify-ticket/?ticket=' . $ticket->ticket_id);

                    $qr = "https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=" . urlencode($verify_url);

                    echo '<div class="ticket-box" style="margin:20px 0;">';
                    echo '<img src="' . $qr . '" />';
                    echo '<p><strong>Ticket ID:</strong> ' . $ticket->ticket_id . '</p>';

                    $calendar_url = site_url('?add_to_calendar=1');

                    echo '<a class="add-to-calendar-btn" href="' . $calendar_url . '" target="_blank">Add to Calendar</a>';
                    echo '</div>';
                }
